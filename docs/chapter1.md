...
##### 0.1.7.6 用 VS Code Remote - Containers 集成开发
...
##### 0.1.10.3 上拉输入（PU）：外部低电平有效、默认需高电平时用
...
###### 0.1.10.3.1 下拉输入（PD）：外部高电平有效、默认需低电平时用
...
##### 0.1.10.4 开漏输出（OD）、复用推挽输出（AF_PP）、复用开漏输出（AF_OD）的使用场景
选择的核心逻辑：
- 是否需要**多设备共享总线**（线与逻辑）→ 选开漏（OD/AF_OD）；
- 是否需要**外设复用引脚**（非普通GPIO控制）→ 选复用（AF_PP/AF_OD）；
- 是否需要**主动拉高电平**（无需外部上拉）→ 选推挽（AF_PP）。

###### 0.1.10.4.1 开漏输出（GPIO_MODE_OUTPUT_OD）：普通GPIO的开漏模式
**核心特性**：
...
###### 0.1.10.4.2 复用推挽输出（GPIO_MODE_AF_PP）：外设复用+推挽输出
**核心特性**：
...
###### 0.1.10.4.3 复用开漏输出（GPIO_MODE_AF_OD）：外设复用+开漏输出
**核心特性**：
...
#### 0.1.11 GPIO工作模式的命名解释
要让学生**通过字面意思+生活比喻**秒记**复用、推挽、开漏**，核心是把专业术语拆成“好懂的名字逻辑”，不用死记硬背，具体拆解如下：

##### 0.1.11.1 一、复用（Alternate Function，AF）——“引脚换主人，功能能复用”
###### 0.1.11.1.1 字面拆解
...
##### 0.1.11.2 二、推挽（Push-Pull，PP）——“能推高、能拉低，主动输出高低”
###### 0.1.11.2.1 字面拆解
...
##### 0.1.11.3 三、开漏（Open-Drain，OD）——“只能拉低，拉高靠外接”
###### 0.1.11.3.1 字面拆解
...
###### 0.1.11.5.2 STM32 CubeMX 时钟树最简默认设置方法
...
###### 0.1.11.5.3 需要特殊设置时钟树的场景
当最简配置无法满足功能/性能需求时，需针对性调整时钟树，常见场景如下：

###### 0.1.11.5.3.1 需求更高的系统主频
...
#### 0.1.12 为什么需要外部时钟源
##### 0.1.12.1 一、STM32不同系列HSI的最高工作频率
...
##### 0.1.12.2 二、要工作到72MHz，是否必须用HSE？
###### 0.1.12.2.1 理论层面：可以不用（但完全不推荐）
...
##### 0.1.12.3 HSE配置72MHz的核心步骤（CubeMX）：
...
###### 0.1.12.4.1 方法一：使用 HAL 库（推荐，STM32CubeIDE）
...
##### 0.1.13.3 Blue Pill 完整电路图（简化原理图）
...
##### 0.1.14.2 要点 2：理解网络标号（Net Label）
...
##### 0.1.14.4 要点 4：读懂芯片引脚图（Pin Diagram）
...
##### 0.1.15.2 电平标准：高电平与低电平

数字电路用电压范围而非精确值来定义"1"和"0"，这是因为实际信号总有噪声和波动。

###### 0.1.15.2.1 TTL 电平（5V 系统，如 Arduino UNO / 51 单片机）
...
###### 0.1.15.2.2 LVTTL / LVCMOS 电平（3.3V 系统，如 STM32）
...
##### 0.1.15.3 三、串口电平（UART / RS-232 / RS-485）
...
##### 0.1.15.4 四、GPIO 驱动能力与输出功率

STM32 的 GPIO 是**弱驱动**接口，设计用于数字信号控制，不能直接驱动大功率负载。

###### 0.1.15.4.1 STM32F103 GPIO 电气参数
...
###### 0.1.15.4.3 三极管驱动电路（GPIO 扩流的基本方法）
...
##### 0.1.15.5 五、发光二极管（LED）的电压与电流

LED 是电流驱动器件（不是电压驱动），必须限流使用，否则会因电流过大烧毁。

###### 0.1.15.5.1 常见 LED 参数
...
##### 0.1.15.7 七、去耦电容（Bypass Capacitor）
...
###### 0.1.16.3.2 UART / RS-232 / RS-485 / CAN 深度对比
...
##### 0.1.18.1 本章在线测试（10 题）

<div id="exam-meta" data-exam-id="chapter1" data-exam-title="第1章 嵌入式基础 测验" style="display:none"></div>
