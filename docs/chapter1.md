...
```text
   ...
   == 1. 输入 (Input) ==           == 2. 加工 (Process) ==          == 3. 输出 (Output) ==   

┌──────────────────────────┐    ┌───────────────────────────┐    ┌──────────────────────────┐
│     传感器与环境感知     │    │     应用软件与算法层      │    │       执行器与动力       │
│ ├ 模拟量: ADC电压采集    │ ─► │ ├ 大脑: 机器学习/SLAM/FSM │ ─► │ ├ 运动控制: 直流减速电机 │
│ ├ 数字量: 超声波/激光雷达│ ─► │ ├ 小脑: PID控制/卡尔曼滤波│ ─► │ ├ 电源控制: 锂电池充放电 │
│ ├ 视听觉: S3视觉 / Mic   │    └───────────────────────────┘    └──────────────────────────┘
└──────────────────────────┘                 │                                │              
             │                               ▼                                ▼              
             ▼                  ┌───────────────────────────┐    ┌──────────────────────────┐
...
│ ├ 中断触发: 外部事件EXTI │ ─► │ ├ 同步: 信号量/消息队列   │    └──────────────────────────┘
│ ├ 界面: 触摸屏坐标采集   │ ─► │ ├ 中断: 强实时延迟处理机制│                                
└──────────────────────────┘    └───────────────────────────┘                                
             │                               │                   ┌──────────────────────────┐
             ▼                               ▼                   │       通信接口发送       │
┌──────────────────────────┐    ┌───────────────────────────┐    │ ├ 串口总线: USART(TX引脚)│
│       通信接口接收       │    │     中间层与硬件架构      │    │ ├ 网络控制: 以太网/ESP32 │
│ ├ 串口总线: USART(RX引脚)│ ─► │ ├ 抽象: BSP / HAL库封装   │ ─► │ ├ 数据存储: SD卡/文件系统│
│ ├ 网络命令: 以太网/WIFI  │ ─► │ ├ 内核: ARM Cortex-M3运算 │ ─► │ ├ 外部互联: I2C/SPI/CAN  │
│ ├ 外部互联: I2C/SPI/CAN  │ ─► │ ├ 传输: DMA直接内存访问   │    └──────────────────────────┘
└──────────────────────────┘    └───────────────────────────┘                                
```
