...
<!-- fig:ch13-1 上图以框图形式描绘了什么是 SLAM 的系统架构，清晰呈现了各模块之间的连接关系与信号流向。 -->
...
<!-- fig:ch13-2 该框图展示了 SLAM 系统架构的核心结构，读者可以从中把握各功能单元的层次划分与协作方式。 -->
...
<!-- fig:ch13-3 上图直观呈现了 Gmapping（基于粒子滤波的 2D SLAM）的组成要素与数据通路，有助于理解系统整体的工作机理。 -->
...
<!-- fig:ch13-4 上图以框图形式描绘了 Cartographer（基于图优化的 2D/3D SLAM）的系统架构，清晰呈现了各模块之间的连接关系与信号流向。 -->
...
<!-- fig:ch13-5 该框图展示了视觉 SLAM 概述的核心结构，读者可以从中把握各功能单元的层次划分与协作方式。 -->
...
<!-- fig:ch13-6 上图直观呈现了 ORB-SLAM 系列的组成要素与数据通路，有助于理解系统整体的工作机理。 -->
...
<!-- tab:ch13-7 视觉惯性 SLAM 融合相机和 IMU，是当前最实用的方案之一： -->
...
<!-- fig:ch13-7 上图以框图形式描绘了选型建议的系统架构，清晰呈现了各模块之间的连接关系与信号流向。 -->
...
<div id="exam-meta" data-exam-id="chapter13" data-exam-title="第十三章 机器人 SLAM 测验" style="display:none"></div>

<!-- mkdocs-quiz intro -->

<quiz>
1) SLAM 问题的本质挑战是什么？
- [x] 定位需要地图、建图需要定位，形成"鸡与蛋"的循环依赖
- [ ] 传感器噪声无法完全消除
- [ ] 地图存储占用大量内存
- [ ] 计算速度不够快

SLAM 的核心困难在于定位和建图的耦合：要在地图中定位自身就需要地图，而要构建地图又需要知道自身位置。EKF-SLAM 和图优化方法通过联合估计机器人位姿和地图来打破这一循环。
</quiz>

<quiz>
2) 基于图优化的 SLAM 后端中，"图"的节点和边分别表示什么？
- [x] 节点表示机器人位姿，边表示位姿间的约束（里程计或回环）
- [ ] 节点表示地图中的路标，边表示路标之间的距离
- [ ] 节点表示传感器数据，边表示时间顺序
- [ ] 节点表示控制输入，边表示状态转移

位姿图中每个节点是机器人在某时刻的位姿 $(x, y, \theta)$，相邻节点之间的边由里程计约束产生，非相邻节点间的边由回环检测产生。后端优化器（如 g2o、GTSAM）通过最小化所有边的误差来获得全局一致的轨迹。
</quiz>

<quiz>
3) 回环检测（Loop Closure）对 SLAM 系统的作用是什么？
- [x] 消除长距离运行累积的漂移误差，使地图全局一致
- [ ] 提高单帧特征提取的速度
- [ ] 减少地图的存储空间
- [ ] 替代里程计提供局部运动估计

里程计和前端 VO 都会随时间累积漂移。当机器人重新访问已到过的地点时，回环检测识别出这一事实并提供额外的约束，后端优化器据此修正整条轨迹，消除累积误差。
</quiz>
...
Gmapping（也称 GMapping）使用 Rao-Blackwellized 粒子滤波，每个粒子维护一个独立的栅格地图。Cartographer 基于子图匹配 + 图优化，ORB-SLAM3 基于关键帧 + BA + 位姿图，LOAM 基于特征点匹配。
</quiz>
