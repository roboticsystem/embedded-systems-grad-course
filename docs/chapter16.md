...
传统机器人依赖人工设计的规则和模型，面对复杂多变的真实环境时存在根本性挑战：
...
上表汇总了为什么机器人需要机器学习中常见的问题现象、可能原因及推荐解决方案，可作为排障时的快速参考手册。<!-- desc-auto -->
...
上图直观呈现了为什么机器人需要机器学习的组成要素与数据通路，有助于理解系统整体的工作机理。<!-- desc-auto -->
...
上图以框图形式描绘了深度学习基础回顾的系统架构，清晰呈现了各模块之间的连接关系与信号流向。<!-- desc-auto -->
...
在第12章我们介绍了目标检测（YOLO）和语义分割。这里补充几个机器人特有的视觉任务：
...
该框图展示了马尔可夫决策过程（MDP）的核心结构，读者可以从中把握各功能单元的层次划分与协作方式。<!-- desc-auto -->
...
通过上表的对比可以看出，不同方案在算法、类型、动作空间等等方面各有优劣，实际选型时应结合具体应用场景综合权衡。<!-- desc-auto -->
...
上图直观呈现了经典控制 vs RL 控制的组成要素与数据通路，有助于理解系统整体的工作机理。<!-- desc-auto -->
...
上表对仿真训练环境中各方案的特性进行了横向对比，便于读者根据实际需求选择最合适的技术路线。<!-- desc-auto -->
...
上图以框图形式描绘了从示范中学习的系统架构，清晰呈现了各模块之间的连接关系与信号流向。<!-- desc-auto -->
...
通过上表的对比可以看出，不同方案在方法、原理、优点等等方面各有优劣，实际选型时应结合具体应用场景综合权衡。<!-- desc-auto -->
...
2023年以来，大语言模型（LLM）和视觉-语言模型（VLM）开始深入机器人领域，形成**具身智能（Embodied AI）**的新范式：
...
该框图展示了大语言模型 + 机器人的核心结构，读者可以从中把握各功能单元的层次划分与协作方式。<!-- desc-auto -->
...
上表对代表性工作中各方案的特性进行了横向对比，便于读者根据实际需求选择最合适的技术路线。<!-- desc-auto -->
...
上图直观呈现了语义 SLAM 与场景图的组成要素与数据通路，有助于理解系统整体的工作机理。<!-- desc-auto -->
...
上图以框图形式描绘了机器学习在机器人各模块的应用总览的系统架构，清晰呈现了各模块之间的连接关系与信号流向。<!-- desc-auto -->
...
通过上表的对比可以看出，不同方案在应用领域、ML 方法、替代/增强的传统方法等等方面各有优劣，实际选型时应结合具体应用场景综合权衡。<!-- desc-auto -->
...
上表对前沿展望中各方案的特性进行了横向对比，便于读者根据实际需求选择最合适的技术路线。<!-- desc-auto -->
