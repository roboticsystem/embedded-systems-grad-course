...
- 理解有限状态机（FSM）的基本概念与核心要素
- 掌握 Switch-Case 和表驱动两种 FSM 实现方式
- 了解层次化状态机（HSM）的设计思想与应用场景
- 能够在嵌入式系统中运用状态机模式解决实际问题

#### 2.3.1 有限状态机原理

**有限状态机（FSM, Finite State Machine）** 是嵌入式软件中最重要的设计模式之一。它将系统行为建模为一组离散状态的集合，通过事件触发状态转移并执行相应动作。FSM 的核心要素包括：

- **状态（State）**：系统在某一时刻所处的稳定情形，如"锁定"、"解锁"、"报警"等
- **事件（Event）**：触发状态转移的外部或内部信号，如按键输入、定时器溢出、通信数据等
- **转移（Transition）**：由当前状态响应事件后迁移到新状态的过程
- **动作（Action）**：状态转移过程中或进入/离开状态时执行的操作，如驱动硬件、更新变量、发送通知等

FSM 的数学定义为一个五元组：`M = (S, E, T, A, s₀)`，其中：
- `S` 是有限状态集合
- `E` 是有限事件集合  
- `T: S × E → S` 是状态转移函数
- `A: S × E → Action` 是动作函数
...
**图 2-3** 智能门锁有限状态机状态转移图。该图展示了系统的四种状态（LOCKED、UNLOCKED、OPEN、ALARM）及其转移关系，每个转移都标注了触发事件和执行动作。
<!-- fig:ch2-3 智能门锁有限状态机状态转移图 -->

#### 2.3.2 Switch-Case 实现

**Switch-Case 实现**是最简单直接的 FSM 编程方式。其核心思想是使用 switch 语句根据当前状态进行分支，在每个分支中处理对应状态下的事件。这种方式代码直观、易于理解，适合状态数量较少（≤5）的简单场景。

以下是智能门锁 FSM 的 Switch-Case 实现代码：
...
当系统状态和事件数量较多时，Switch-Case 实现会导致代码冗长、维护困难。**表驱动 FSM** 将状态转移关系抽象为数据结构，通过查表方式实现状态转移，代码结构更清晰、扩展性更好。

表驱动 FSM 的核心思想是：
1. 定义状态转移表数据结构，包含目标状态和动作函数指针
2. 将所有状态转移规则存储在表中
3. 处理事件时，通过当前状态和事件索引查表获取转移信息

以下是智能门锁 FSM 的表驱动实现代码：
...
**表 2-3** Switch-Case 与表驱动 FSM 实现对比
<!-- tab:ch2-3 -->

| 特性 | Switch-Case 实现 | 表驱动实现 |
|------|-----------------|-----------|
| **代码可读性** | 直观易懂，适合初学者 | 结构化设计，规则一目了然 |
| **扩展性** | 新增状态需修改 switch 分支 | 只需扩展状态转移表 |
| **性能** | 略高（直接分支跳转） | 稍低（需数组索引查找） |
| **适用场景** | 状态数较少（≤5）的简单系统 | 状态/事件较多的复杂系统 |
| **可维护性** | 状态增多时代码臃肿 | 规则集中管理，易于维护 |
| **代码量** | 状态越多代码量增长越快 | 代码量增长平缓 |

表驱动方式的核心优势在于将状态转移规则从代码逻辑中分离出来，实现了**数据与逻辑的分离**，这是一种重要的软件工程思想。

#### 2.3.4 层次化状态机（HSM）简介

在复杂系统中，多个状态常常共享相同的行为逻辑。例如，一个智能设备可能有"正常运行"和"充电"两种工作模式，但两种模式下都需要处理错误事件。**层次化状态机（Hierarchical State Machine, HSM）** 通过状态继承机制消除这种重复。

HSM 的核心思想是将状态组织成层次结构：
- **父状态**：定义多个子状态共享的公共行为
- **子状态**：只处理与父状态不同的特殊行为
- **状态继承**：子状态可以继承父状态的事件处理逻辑
...
HSM 的主要优势包括：
- **代码复用**：公共行为只需在父状态中定义一次
- **层次清晰**：状态关系一目了然，便于理解复杂系统
- **扩展性好**：新增子状态只需定义差异行为
- **维护性强**：修改公共行为只需修改父状态

#### 2.3.5 本节小结

有限状态机是嵌入式系统中处理复杂逻辑的核心设计模式，本章介绍了三种实现方式：

1. **Switch-Case 实现**：简单直观，适合状态数较少的场景（≤5个状态）
2. **表驱动实现**：将状态转移规则抽象为数据结构，实现数据与逻辑分离，适合状态/事件较多的复杂系统
3. **层次化状态机（HSM）**：通过状态继承机制消除重复代码，适合具有层次关系的复杂状态系统

选择合适的实现方式需要综合考虑系统复杂度、性能要求和可维护性需求。在实际项目中，表驱动方式是最常用的选择，因为它兼顾了可读性和扩展性。

#### 2.3.6 PicSimLab 仿真演示

为帮助读者更好地理解状态机的运行机制，本节提供基于 PicSimLab 的仿真演示步骤。
...
1. **启动仿真**：执行上述命令启动 PicSimLab
2. **初始状态**：系统处于锁定状态（LOCKED），PB0 LED 点亮
3. **输入正确密码**：依次按下 PA0、PA1、PA2 按键（模拟密码输入）
4. **状态转移**：密码正确后，系统转移到解锁状态（UNLOCKED），PB1 LED 点亮
5. **触发报警**：连续输入错误密码3次，系统进入报警状态（ALARM），PB2 LED 点亮，蜂鸣器响
6. **复位系统**：按下复位按钮，系统回到锁定状态
