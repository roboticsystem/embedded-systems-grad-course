...
STM32 定时器的时钟源来自 APB 总线，经预分频后进入计数器：

```mermaid
flowchart LR
    HSE[外部晶振 HSE] --> PLL[PLL 锁相环]
    HSI[内部 RC HSI] --> PLL
    PLL --> SYSCLK[系统时钟 SYSCLK]
    SYSCLK --> AHB[AHB 总线]
    AHB --> APB1[APB1 总线 ≤42MHz]
    AHB --> APB2[APB2 总线 ≤84MHz]
    APB1 --> TIM2_7[TIM2~7 定时器时钟]
    APB2 --> TIM1_8[TIM1/TIM8 定时器时钟]
```

**重要**：当 APB 分频系数 > 1 时，定时器时钟 = APB 时钟 × 2。例如：
- SYSCLK = 168MHz, AHB = 168MHz, APB1 分频 = 4 → APB1 = 42MHz → TIM2~7 时钟 = 84MHz
- APB2 分频 = 2 → APB2 = 84MHz → TIM1/8 时钟 = 168MHz

### 7.2.2 定时器核心寄存器

| 寄存器 | 简称 | 作用 |
|---|---|---|
| 预分频器 | PSC | 将时钟分频，计数器时钟 = 定时器时钟 / (PSC + 1) |
| 自动重装载 | ARR | 设定计数上限，达到后产生更新事件 |
| 计数器 | CNT | 当前计数值 |
| 捕获/比较 | CCR1~4 | 用于输出比较（PWM 占空比）或输入捕获（记录时间戳） |
| 重复计数器 | RCR | 高级定时器专有，设定更新事件的触发频率分频 |

### 7.2.3 定时器定时计算公式

**定时周期计算**：

$$T_{overflow} = \frac{(PSC + 1) \times (ARR + 1)}{f_{timer}}$$

**举例**：若定时器时钟 = 84MHz，需要 1ms 定时中断：

$$1\text{ms} = \frac{(PSC + 1) \times (ARR + 1)}{84 \times 10^6}$$

可取 PSC = 83, ARR = 999：$(83+1) \times (999+1) = 84000$，$84000 / 84MHz = 1ms$ ✓

也可取 PSC = 8399, ARR = 9：$(8400) \times (10) = 84000$，同样 1ms ✓

**选择原则**：PSC 大 → 分辨率低但计数范围大；PSC 小 → 分辨率高但 ARR 需更大。

## 7.3 计数模式

### 7.3.1 向上计数（Upcounting）

计数器从 0 递增到 ARR，产生更新事件后复位到 0：
...
最常用的计数模式，适合大多数定时和 PWM 场景。

### 7.3.2 向下计数（Downcounting）

计数器从 ARR 递减到 0，产生更新事件后重装为 ARR。使用较少，某些特殊场景可能需要。

### 7.3.3 中心对齐模式（Center-aligned）

计数器从 0 递增到 ARR-1，再从 ARR 递减到 1，在两个端点均可产生更新事件：
...
主要用途：生成对称 PWM，减少电机驱动中的谐波分量，适合半桥/全桥电机控制。

### 7.3.4 模式对比

| 模式 | 更新事件频率 | PWM 波形 | 适用场景 |
|---|---|---|---|
| 向上计数 | 每 ARR+1 个时钟一次 | 非对称（边沿对齐） | 通用定时、标准 PWM |
| 向下计数 | 每 ARR+1 个时钟一次 | 非对称 | 特殊触发需求 |
| 中心对齐 | 每 2×ARR 个时钟一次 | 对称（中心对齐） | 电机控制、互补 PWM |
